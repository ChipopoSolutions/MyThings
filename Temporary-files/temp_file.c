...
temp_file_handle write_temp_file (char* buffer, size_t length)
{
    ...
    char filename_template[] = "/tmp/chipopoXXXXXX";
...
{
    int f_dsc;
    char* str_buffer="AaBbCcDdEe\n";
    size_t* ptr_str_len=(size_t *) malloc (sizeof (size_t));

    * ptr_str_len = 11;

    f_dsc=write_temp_file(str_buffer,*ptr_str_len);

    printf("press any key to continue...");
    getchar();


    char* r_buffer=read_temp_file(f_dsc, ptr_str_len);
    printf("buffer=%s",r_buffer);
    free(ptr_str_len);
}
